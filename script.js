...
const validateRows = () => {
  ...
  for (let i = 1; i <= 9; i++) {
    var row_ar = [];
    for (let j = 1; j <= 9; j++) {
      var ipt1 = document.querySelector(`#r${i} #d${j} input`);
      var ipt = ipt1.value;
      ...
      if (ipt != "") {
        if (row_ar.indexOf(ipt) == -1) 
          ...
          var nmbr = document.querySelector(
            `#r${i} #d${row_ar.indexOf(ipt) + 1} input`
          );
...
const validateColumn = () => {
    ...
    for (let i = 1; i <= 9; i++) {
      var row_ar = [];
      for (let j = 1; j <= 9; j++) {
        var ipt1 = document.querySelector(`#r${j} #d${i} input`);
        var ipt = ipt1.value;
        if (ipt != "") {
          if (row_ar.indexOf(ipt) == -1) 
            row_ar.push(ipt);
            ...
            flag = 1;
            var nmbr = document.querySelector(
              `#r${row_ar.indexOf(ipt) + 1} #d${i} input`
            );
            row_ar.push(ipt);
            nmbr.style.color = "red";
            ipt1.style.color = "red";
  ...
  for(var i1=a;i1<a+3;i1++){
    for(var j1=b;j1<b+3;j1++){
      if(full_data[i][j]==full_data[i1][j1]&&(i!=i1&&j!=j1)&&full_data[i1][j1]!==""){
        var ipt1 = document.querySelector(`#r${i+1} #d${j+1} input`);
        ipt1.style.color='red';
        ipt1 = document.querySelector(`#r${i1+1} #d${j1+1} input`);
        ipt1.style.color='red';
        c++;}}}}}
  if(c!=0)  return false;
  return true;
